...
async def main_async():
    ...
    try:
        ...
        print(f"[{datetime.utcnow().isoformat()}Z] websockets not installed — running stdio bridge", flush=True)
        for line in sys.stdin:
            try:
                req = json.loads(line); rid = req.get("id"); tool = req.get("tool"); args = req.get("args", {}) or {}
                if tool == "tools/list":
                    print(json.dumps({"id": rid, "ok": True, "result": list(TOOLS.keys())}), flush=True); continue
                if tool not in TOOLS:
                    print(json.dumps({"id": rid, "ok": False, "error": f"unknown tool {tool}"}), flush=True); continue
