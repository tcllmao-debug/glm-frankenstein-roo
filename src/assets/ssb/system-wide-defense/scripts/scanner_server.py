...
UPLOAD_DIR = Path(os.environ.get("SSB_DEFENSE_UPLOAD_DIR", "/home/z/my-project/uploads"))
try:
    UPLOAD_DIR.mkdir(parents=True, exist_ok=True)
except OSError:
    # ADDITIVE v13: portable fallback when the original author's home is absent
    UPLOAD_DIR = Path(os.environ.get("SSB_DEFENSE_HOME", "/tmp/ssb_defense")) / "uploads"
    UPLOAD_DIR.mkdir(parents=True, exist_ok=True)
...
<body>
  ...
  <div class="toggle-section">
    <h2>[] Self-Heal Toggle</h2>
    <p>
      Click to PAUSE self-heal + baseline protection before applying legitimate updates.
      While paused, the self-heal system will NOT revert file changes. Click again to RESUME.
      Use /api/self-heal/rebaseline after updates to adopt the new state.
    </p>
    <button class="toggle-btn" id="healToggleBtn" onclick="toggleHeal()">
      PAUSE SELF-HEAL FOR UPDATES
    </button>
    <div class="status-text" id="healStatus">
      Self-heal is: <span class="status-active">ACTIVE</span>
    </div>
  </div>
...
async function updateStats() {
  ...
  if (isPaused) {
    btn.textContent = 'RESUME SELF-HEAL';
    btn.classList.add('paused');
    statusDiv.innerHTML = 'Self-heal is: <span class="status-paused">PAUSED</span> — updates allowed';
  } else {
    btn.textContent = 'PAUSE SELF-HEAL FOR UPDATES';
    btn.classList.remove('paused');
    statusDiv.innerHTML = 'Self-heal is: <span class="status-active">ACTIVE</span>';
  }
}
...
async function toggleHeal() {
  ...
  const r = await fetch('/api/self-heal/toggle', {method: 'POST'});
