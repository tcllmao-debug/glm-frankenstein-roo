...
export class ZAiHandler extends BaseOpenAiCompatibleProvider<string> {
	constructor(options: ApiHandlerOptions) {
		// GLM Frankenstein: default to international_api endpoint so users who
		// fetch their API key from the z.ai web page (https://z.ai) work out of
		// the box. The coding-specific endpoints only accept keys issued from
		// https://open.bigmodel.cn — flipping the default avoids the common
		// "401 Invalid api key" surprise for international users.
		const defaultApiLine = options.zaiApiLine ?? "international_api"
		const isChina = zaiApiLineConfigs[defaultApiLine].isChina
		...
		super({
			...options,
			providerName: "Z.ai",
			baseURL: zaiApiLineConfigs[defaultApiLine].baseUrl,
