...
export interface SecretCandidate { type: string; value: string; file: string; line: number; context: string }
export interface EntropyResult { entropy: number; maxEntropy: number; efficiency: number; distribution: Record<string, { count: number; probability: number }>; proof: string }
export interface ReviewVerdict { reviewer: "AI" | "OpenClaw" | "HermesBeastClaw"; classification: "real_secret" | "false_positive" | "suspicious"; confidence: number; reasoning: string }
export interface TripleReviewResult { candidate: SecretCandidate; entropy: EntropyResult; reviews: ReviewVerdict[]; finalClassification: "real_secret" | "false_positive" | "suspicious"; finalConfidence: number }

export function shannonEntropy(value: string): EntropyResult {
  if (!value) return { entropy: 0, maxEntropy: 0, efficiency: 0, distribution: {}, proof: "Empty string." }
  ...
  const proof = `Shannon Entropy: ${entropy.toFixed(6)} bits/char, efficiency ${efficiency.toFixed(1)}%, length ${n}, unique ${freq.size}`
...
export function openClawReview(candidate: SecretCandidate, entropy: EntropyResult): ReviewVerdict {
  ...
  let score = 70
  if (entropy.entropy >= 3.0) { reasons.push(`Entropy OK.`); score += 10 }
...
export function hermesBeastClawReview(candidate: SecretCandidate, entropy: EntropyResult): ReviewVerdict {
  const reasons: string[] = ["Hermes Beast Claw: assume false positive."]
  let score = 35
  if (/(example|sample|test|demo|placeholder|fake|dummy|xxxx|foo|bar)/i.test(candidate.value)) { reasons.push("Placeholder pattern."); score -= 30 }
  if (/\.(test|spec|mock|fixture)\./.test(candidate.file)) { reasons.push("Test/mock file."); score -= 25 }
  if (entropy.entropy < 3.0) { reasons.push(`Entropy too low.`); score -= 20 }
  if (entropy.entropy >= 4.5 && candidate.value.length >= 32) { reasons.push("Concede real."); score += 35 }
  if (/-----BEGIN .*PRIVATE KEY-----/.test(candidate.value)) { reasons.push("PEM block — concede."); score += 40 }
  const classification = score >= 65 ? "real_secret" : score >= 45 ? "suspicious" : "false_positive"
...
function parseLlmVerdict(text: string, reviewer: "OpenClaw" | "HermesBeastClaw", fallback: ReviewVerdict): ReviewVerdict {
  try {
    ...
    return { reviewer, classification, confidence: Math.min(100, Math.max(0, Number(parsed.confidence ?? fallback.confidence))), reasoning: parsed.reasoning ?? fallback.reasoning }
...
export async function openClawReviewGlm(candidate: SecretCandidate, entropy: EntropyResult, api: ApiHandler): Promise<ReviewVerdict> {
  ...
  const sys = `You are OpenClaw, a conservative security reviewer for the SSB triple-review pipeline. You authenticate via the configured Z.ai GLM-5.2 key (International Coding endpoint). Default stance: "real secret" unless strong evidence proves otherwise. Respond with strict JSON: {"classification":"real_secret|suspicious|false_positive","confidence":0-100,"reasoning":"one sentence"}.`
...
export async function hermesBeastClawReviewGlm(candidate: SecretCandidate, entropy: EntropyResult, api: ApiHandler): Promise<ReviewVerdict> {
  ...
  if (!api) return fallback
  const sys = `You are Hermes Beast Claw, an adversarial security reviewer for the SSB triple-review pipeline. You authenticate via the configured Z.ai GLM-5.2 key (International Coding endpoint). Default stance: "false positive" — try to prove the secret is fake. Respond with strict JSON: {"classification":"real_secret|suspicious|false_positive","confidence":0-100,"reasoning":"one sentence"}.`
  const user = `Secret candidate:
  type: ${candidate.type}
  value: ${candidate.value.slice(0, 80)}${candidate.value.length > 80 ? "..." : ""}
  file: ${candidate.file}:${candidate.line}
...
export async function tripleReviewGlm(candidate: SecretCandidate, api?: ApiHandler): Promise<TripleReviewResult> {
  ...
  let openClaw: ReviewVerdict, hermes: ReviewVerdict
  if (api) {
    [openClaw, hermes] = await Promise.all([openClawReviewGlm(candidate, entropy, api), hermesBeastClawReviewGlm(candidate, entropy, api)])
  } else {
...
const IGNORE_DIRS = new Set(["node_modules", ".git", "dist", "build", "out", ".next", ".vscode", ".idea", "coverage", ".cache", "__pycache__", ".pytest_cache", ".venv", "venv", "env", ".mypy_cache", ".turbo", ".parcel-cache", "target", ".gradle", ".stack-work", "logs", "pids", "scanner_data"])
const IGNORE_EXTS = new Set([".png", ".jpg", ".jpeg", ".gif", ".bmp", ".ico", ".svg", ".pdf", ".zip", ".tar", ".gz", ".rar", ".7z", ".mp4", ".mp3", ".avi", ".mov", ".wav", ".so", ".dll", ".exe", ".bin", ".class", ".wasm", ".lock", ".min.js", ".min.css"])

export async function scanPathGlm(rootPath: string, api?: ApiHandler, maxFiles = 1000, onProgress?: (msg: string) => void): Promise<TripleReviewResult[]> {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        try {
          ...
          filesScanned++
          candidates.push(...findSecrets(content, path.relative(rootPath, full)))
        } catch {}
      }
    }
  }
  onProgress?.(`Found ${candidates.length} secret candidate(s). Reviewing…`)
...
export function formatReviewResult(results: TripleReviewResult[]): string {
  ...
  for (const r of results) {
    ...
    lines.push(`Entropy: ${r.entropy.entropy.toFixed(3)} bits/char`)
