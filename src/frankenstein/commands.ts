...
async function pickCommit(cwd: string): Promise<string | undefined> {
  ...
  let log: string
  try { log = execSync("git log --oneline -20", { cwd, encoding: "utf8" }) } catch { vscode.window.showErrorMessage("git log failed."); return }
...
export async function reviewUncommittedCommand() {
  ...
  try {
    const result = await reviewUncommitted(glm, cwd, (p) => vscode.window.setStatusBarMessage(`GLM Frankenstein: ${p}`, 3000))
...
export async function reviewCommitCommand() {
  ...
  if (Object.keys(files).length === 0) { vscode.window.showInformationMessage(`Commit ${sha} has no diff.`); return }
  ...
  const sys = `You are CodeRabbit-GLM. Reply with a strict JSON array of issues.`
  const allComments: any[] = []
  for (const hunk of allHunks) allComments.push(...await reviewHunkWithGLM(glm, hunk, sys))
...
export async function ssbStartCommand() {
  ...
  vscode.window.showInformationMessage("GLM Frankenstein: SSB stack start requested. Check Output → 'GLM Frankenstein SSB' channel.")
