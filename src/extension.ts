...
import { SsbService } from "./frankenstein/SsbService"
...
export async function activate(context: vscode.ExtensionContext) {
        ...
        // GLM Frankenstein: stash context globally so Frankenstein command module can reach it.
        (global as any).__glmFrankensteinContext = context
        ...
        // GLM Frankenstein: autostart the SSB Python stack (scanner, persistent
        // connection, activation daemon, triple-review server, MCP bridge, CLI
        // omni-runner, Next.js portal). Best-effort — never blocks activation.
        void SsbService.getInstance().autostart().catch((e) =>
                outputChannel.appendLine(`[SSB] Autostart error: ${e instanceof Error ? e.message : String(e)}`),
        )
...
export async function deactivate() {
        ...
        // GLM Frankenstein: stop the SSB stack
        try { await SsbService.getInstance().stop() }
        catch (e) { outputChannel.appendLine(`[SSB] Stop error: ${e instanceof Error ? e.message : String(e)}`) }
